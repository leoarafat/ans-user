  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await addTikTokClaimRequest({
        ...
        ugclink: formData.get("ugclink") as string,
        pgcLink: formData.get("pgcLink") as string,
        timeForPgc: formData.get("timeForPgc") as string,
        timeForUgc: formData.get("timeForUgc") as string,
      });
      if (res?.data?.success === true) {
        toast.success("Success");
      }
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                id="timeForPgc"
                name="timeForPgc"
                label="Time frame of the PGC that matches the UGC"
                variant="outlined"
                sx={{ backgroundColor: "#f9f9f9", borderRadius: "8px" }}
              />
            </Grid>
